...
- **前端依赖全面升级（minor/patch）**：`npm update` 升级至 semver 范围内最新稳定版——vue `3.5.41→3.5.43`、vite `8.2.2→8.3.0`、axios `1.19.0→1.20.0`、element-plus `2.14.4→2.14.6`、vue-router `5.2.0→5.3.1`、vue-echarts `8.1.0→8.3.0`、sass、@vitejs/plugin-vue。`npm audit` 0 漏洞、`vite build` 通过（element-plus 分包修复保留）、冒烟 15/15。后端 Spring Boot 4.1.1 / MyBatis-Plus 3.5.17 已是最新稳定版。
